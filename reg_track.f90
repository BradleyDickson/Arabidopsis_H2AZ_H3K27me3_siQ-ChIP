      real*8 bin(100000),sum,itops,scalerx,bcount
...
!            read(arg,*)scalerx
      ...
      if(k.eq.1)open(88,file='NC.'//TRIM(path(2)))
...
!      if(unk /= "chr1")go to 54321 
      ...
      if(SCAN(unk(5:6), "_").eq.0.and.unk/='chrM')then
...
!                  write(88,*) unk, lpos-nleng(k)/2, dble(sum)/dble(nleng(k))
            ...
            do i=1,rightend-leftend
               bin(i)=bin(i)+1d0/dble(leng*bcount)
